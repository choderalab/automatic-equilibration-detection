...
This effect leads to significantly biased estimates of the expectation unless simulations are sufficiently long to eliminate starting point dependent bias, which takes a surprisingly long $\sim$30 ns in this example.
...
To show that this approach is indeed general, we repeated the analysis illustrated above in Figs.~\ref{figure:burn-in-example}--\ref{figure:argon-rmse} for a different choice of observable $A(x)$ for the same liquid argon system---in this case, the reduced potential energy $u^*(x) \equiv \beta [U(x) + p V(x)]$.
...
\begin{figure*}[tbp]
\subfigure[\emph{Top:} Average reduced potential for relaxation from initial conditions. \emph{Bottom:} Cumulative averages of reduced potential with and without discarding some initial data to equilibration.]{\resizebox{0.9\textwidth}{!}{\includegraphics{argon-potential.pdf}}}
...
The analyses of Figs.~\ref{figure:burn-in-example}--\ref{figure:argon-rmse} were repeated for the reduced potential energy $u^*(x) \equiv \beta [U(x) + p V(x)]$ of the liquid argon system.  
