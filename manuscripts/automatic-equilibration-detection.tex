...
\end{eqnarray}
We quantify the bias in an estimator $\hat{A}$ by the expected error $\delta(\hat{A})$,
\begin{eqnarray}
\delta(\hat{A}) &\equiv& \left| \expect{\hat{A}} - \expect{A} \right| . \label{equation:error} 
\end{eqnarray}
In this note, we concern ourselves with this question: 
Is there a simple approach to choosing an optimal equilibration time $t_0$ that provides an improved estimate $\hat{A}_{[t_0,T]}$ such that $\delta(\hat{A}_{[t_0,T]}) < \delta(\hat{A}_{[1,T]})$?

While several automated methods for selecting $t_0$, these approaches have shortcomings that have greatly limited their use.
